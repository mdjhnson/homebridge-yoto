...
import { fetchFamilyLibrary, fetchMyoCards, mergeLibraryCards } from './utils/library.js'
...
export class YotoPlatform {
  ...
  /**
   * Cards in the family library plus the user's Make Your Own cards, for TV
   * inputs. Cached briefly; resolves to [] if the library can't be read.
  ...
  getLibraryCards () {
    ...
    /**
     * @param {string} what
     * @param {Promise<LibraryCard[]>} request
     * @returns {Promise<LibraryCard[]>}
     */
    const tolerate = (what, request) => request.catch(error => {
      this.log.warn(`Could not load ${what} for TV inputs (${errorMessage(error)}).`)
      return []
    })

    const cards = Promise.all([
      tolerate('the Yoto card library', fetchFamilyLibrary(client)),
      tolerate('Make Your Own cards', fetchMyoCards(client)),
    ]).then(lists => {
      const merged = mergeLibraryCards(...lists)
      for (const card of merged) {
        if (!this.cardTitleCache.has(card.cardId)) {
          this.cardTitleCache.set(card.cardId, Promise.resolve(card.title))
        }
      }
      this.log.debug(`Loaded ${merged.length} library card(s) for TV inputs`)
      return merged
    })
