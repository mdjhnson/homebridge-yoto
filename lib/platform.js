...
export class YotoPlatform {
   ...
   * when Homebridge boots, retry with backoff instead of giving up.
  ...
  async connectAccount () {
    ...
    try {
      ...
      if (this.authInvalid) {
        this.log.error('Failed to start account:', message)
        return
      }
      const delayMs = Math.min(START_RETRY_MAX_MS, START_RETRY_BASE_MS * 2 ** this.startRetryCount)
      this.startRetryCount++
      this.log.warn(`Could not connect to Yoto (${message}). Retrying in ${Math.round(delayMs / 1000)} seconds.`)
