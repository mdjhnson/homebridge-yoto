...
export class YotoTelevisionAccessory {
  ...
  setupInputSources () {
    ...
    for (const entry of shortcuts) {
      identifier += 1
      const fallbackName = `Shortcut ${entry.number}`
      const service = this.configureInputSource(`ShortcutInput:${entry.id}`, identifier, fallbackName)
      /** @type {PlayableCard} */
      const card = {
        label: fallbackName,
        cardId: entry.cardId,
      ...
      this.resolveInputName(service, card)
    }
   ...
   * @returns {Service}
   */
  configureInputSource (subtype, identifier, name) {
    const { Service, Characteristic } = this.#platform
    ...
    if (!existing || subtype === 'PlaybackInput' || subtype.startsWith('CardInput:')) {
      service
  ...
  async setActiveIdentifier (value) {
    ...
    try {
      ...
      await this.#deviceModel.startCard({
        cardId: card.cardId,
        ...(card.chapterKey !== undefined && { chapterKey: card.chapterKey }),
        ...(card.trackKey !== undefined && { trackKey: card.trackKey }),
      })
    } catch (error) {
