...
import { allocateInputIdentifier, encodeDisplayOrder, sortByName } from './utils/input-order.js'
import {
  clampPercent,
  clampSteps,
  percentToSteps,
...
const INPUT_SUBTYPE_PREFIXES = ['CardInput:', 'ShortcutInput:', 'LibraryInput:']
/**
...
export class YotoTelevisionAccessory {
  ...
  setupInputSources () {
    ...
    const usedIdentifiers = new Set([NOW_PLAYING_INPUT_ID])
    ...
    for (const control of getCardControlConfigs(this.#platform.config)) {
      const subtype = `CardInput:${control.id}`
      const identifier = allocateInputIdentifier(subtype, usedIdentifiers)
      const service = this.configureInputSource(subtype, identifier, control.label)
    ...
    for (const entry of shortcuts) {
      const subtype = `ShortcutInput:${entry.id}`
      const identifier = allocateInputIdentifier(subtype, usedIdentifiers)
      const fallbackName = entry.builtInName ?? `Shortcut ${entry.number}`
      const service = this.configureInputSource(subtype, identifier, fallbackName, Boolean(entry.builtInName))
    ...
    const room = Math.max(0, MAX_TV_INPUTS - inputs.size)
    for (const libraryCard of libraryCards.slice(0, room)) {
      const subtype = `LibraryInput:${libraryCard.cardId}`
      const identifier = allocateInputIdentifier(subtype, usedIdentifiers)
      const service = this.configureInputSource(subtype, identifier, libraryCard.title, true)
      inputs.set(identifier, { service, card: { label: libraryCard.title, cardId: libraryCard.cardId } })
    }
  ...
  /**
   * Fetch the card library and rebuild the inputs if it changed.
   */
  loadLibraryInputs () {
    this.#platform.getLibraryCards().then(cards => {
      ...
      this.#log.debug(LOG_PREFIX.ACCESSORY, `[${this.#device.name}] Failed to load library TV inputs:`, formatError(error))
    })
  }
