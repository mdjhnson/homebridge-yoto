...
/**
 * Content a switch can start playing.
...
export class YotoPlayerAccessory {
  ...
  setupShortcutServices () {
    ...
    for (const entry of entries) {
      ...
      const fallbackName = this.generateServiceName(`Shortcut ${entry.number}`)
      ...
      this.#currentServices.add(service)
      this.resolveShortcutName(service, entry)
    }
  ...
  getShortcutPlayable (service, entry) {
    return {
      label: service.displayName || `Shortcut ${entry.number}`,
      cardId: entry.cardId,
  ...
  async setCardControl (service, control, value) {
    ...
    try {
      await this.#deviceModel.startCard({
        ...
        ...(control.chapterKey !== undefined && { chapterKey: control.chapterKey }),
        ...(control.trackKey !== undefined && { trackKey: control.trackKey }),
      })
    } catch (error) {
      this.#log.error(
        LOG_PREFIX.ACCESSORY,
