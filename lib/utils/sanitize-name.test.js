...
test('sanitizeName strips disallowed characters and emojis', () => {
  ...
  assert.equal(sanitizeName('Story time... -'), 'Story time.')
  assert.equal(sanitizeName('Songs & -'), 'Songs')
...
test('sanitizeName shortens names to 64 characters at a word break', () => {
  ...
  const cutAtAmpersand = sanitizeName('Peppa Pig and George Pig Stories for Bedtime and Morning Tales & Songs Forever')
  assert.equal(cutAtAmpersand, 'Peppa Pig and George Pig Stories for Bedtime and Morning Tales')
