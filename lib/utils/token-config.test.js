...
test('falls back to the single Yoto block when previous tokens do not match', () => {
  const config = JSON.stringify({ platforms: [{ platform: 'Yoto', accessToken: 'edited-by-ui' }] })
  const result = applyTokenUpdate(config, { ...baseUpdate, prevAccessToken: 'stale' })
  assert.ok(result)
